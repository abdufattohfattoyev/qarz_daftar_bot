...
export default function Home() {
  ...
  return (
    // Tepa qism (sarlavha + Muddatlar) qotib turadi, faqat pastdagi ro'yxat scroll bo'ladi
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: c.bg }}>

      {/* ══════ QOTIB TURADIGAN QISM ══════ */}
      <div style={{ flexShrink: 0, position: 'relative', zIndex: 5 }}>
        ...
        {/* ⏰ MUDDAT OGOHLANTIRISHLARI — shu yergacha qotib turadi */}
        {dueAlerts.length > 0 && !q && (
          <div style={{ background: c.bg, padding: '10px 0 4px', borderBottom: `1px solid ${c.border}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 16px 8px' }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: c.text }}>⏰ {t('due_title')}</span>
              ...
              {dueAlerts.map((d) => {
                ...
                return (
                  <div key={d.id} onClick={() => { haptic('light'); setSheetDebt(d) }} className="list-item" style={{
                    minWidth: 156, flexShrink: 0, background: c.card, borderRadius: 16, padding: 12,
                    border: `1.5px solid ${color}33`, boxShadow: c.shadowSm, cursor: 'pointer',
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      ...
                      <div style={{ fontSize: 12, fontWeight: 700, color: c.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.contact_name}</div>
                    </div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: isGave ? '#16a34a' : '#ef4444', letterSpacing: -.3 }}>
                      {isGave ? '+' : '−'}{n(d.remaining_amount)} <span style={{ fontSize: 9, fontWeight: 600, color: c.faint }}>{d.currency}</span>
                    </div>
      ...
      <div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', paddingBottom: 80 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px 6px' }}>
          <h3 style={{ margin: 0, fontSize: 13, fontWeight: 700, color: c.text }}>{t('recent_ops')}</h3>
        ...
        {debts.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, margin: '4px 16px 8px', padding: '9px 12px', background: c.card, borderRadius: 13, border: `1.5px solid ${c.border}` }}>
            ...
            <input
              ...
              style={{ border: 'none', background: 'transparent', fontSize: 13, color: c.text, fontFamily: 'inherit', outline: 'none', flex: 1, minWidth: 0 }}
            />
            {search && (
              <button onClick={() => setSearch('')} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 2 }}>
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <circle cx="7" cy="7" r="6" fill={c.chip}/>
                  <path d="M4.5 4.5l5 5M9.5 4.5l-5 5" stroke={c.muted} strokeWidth="1.4" strokeLinecap="round"/>
                </svg>
              </button>
            )}
          </div>
        ...
        {!loading && groups.length === 0 && q && (
          <div style={{ textAlign: 'center', padding: '40px 24px', fontSize: 13, color: c.muted }}>
        ...
        {!loading && groups.length === 0 && !q && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '36px 24px' }}>
            ...
            <p style={{ margin: '14px 0 4px', fontSize: 15, fontWeight: 800, color: c.text }}>{t('no_debts')}</p>
            <p style={{ margin: '0 0 18px', fontSize: 12, color: c.muted, textAlign: 'center' }}>{t('add_first_debt')}</p>
        ...
        {groups.map((group) => (
          <div key={group.date} style={{ marginBottom: 4 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 16px 4px' }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: c.muted, textTransform: 'uppercase', letterSpacing: '.05em' }}>{fmtDate(group.date)}</span>
            ...
            <div style={{ margin: '0 12px', background: c.card, borderRadius: 16, overflow: 'hidden', boxShadow: c.shadowSm }}>
              {group.debts.map((debt, idx) => {
                ...
                return (
                  <div key={debt.id} onClick={() => { haptic('light'); setSheetDebt(debt) }} className="list-item" style={{
                    ...
                    borderBottom: idx < group.debts.length - 1 ? `1px solid ${c.border}` : 'none',
                    background: isPaid ? c.card2 : 'transparent',
                  }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 }}>
                        ...
                        <span style={{ fontSize: 10, color: c.faint }}>{fmtTime(debt.created_at)}</span>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      ...
                      <p style={{ margin: '1px 0 0', fontSize: 9, color: c.faint }}>{debt.currency}</p>
                    </div>
                    <ChevronRight />
                  </div>
                )
